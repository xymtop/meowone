...
async def run_due_scheduled_tasks(limit: int = 5) -> Dict[str, Any]:
    ...
    from app.dispatch.gateway import dispatch as dispatch_gateway
    ...
    for row in rows:
        ...
        try:
            ...
            if agent_row is None:
                ...
                parts: List[str] = []
                async for event in dispatch_gateway(user_message=str(item["prompt"]), history=[], agent_name=agent_name):
                    ev_type = type(event).__name__
                    if ev_type == "DeltaEvent" and getattr(event, "content", ""):
                        parts.append(event.content)
                output = "".join(parts)
                invoke_result = {"ok": True, "output": output}
                ok = True
            async with get_db() as db:
                await db.execute(
                    """
