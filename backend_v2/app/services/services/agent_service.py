...
def _to_dict(row: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        data["metadata_json"] = {}
    # 从 metadata_json 中提取 model_name、scheduler_mode 和 loop_mode
    ...
    data["loop_mode"] = metadata.get("loop_mode", "react") if isinstance(metadata, dict) else "react"
...
async def list_external_agents() -> List[ExternalAgentEntry]:
    async with get_db() as db:
        cur = await db.execute(
            """
