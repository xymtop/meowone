...
The product renders **A2UI v0.8** **JSON messages** (see https://a2ui.org — transport is normally a JSON stream, not executable code). Do **not** invent ad-hoc JSON UI protocols in prose.
- **MeowOne embedding (preferred):** put the JSON between **`---A2UI-START---`** and **`---A2UI-END---`** on their own lines so the user does **not** see raw JSON in the transcript. Alternatively you may use a Markdown fenced block with language **`a2ui`**.
...
- **User messages from the UI:** the client sends **`[A2UI Action] {{ ... }}`** over the chat API. This includes **`Button`** `action` payloads, and in MeowOne also **`name`: `meowone.selectionChange`** when the user changes a **MultipleChoice** value (read **`context`** / **`value`**). Interpret these and continue the task.
