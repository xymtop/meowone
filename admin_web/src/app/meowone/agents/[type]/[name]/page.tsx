...
import { useEffect, useState, useCallback, Suspense, ReactElement } from "react";
...
function FileTextIcon() {
  return (
    <svg className="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 0V6.75m0 0H4.875c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
    </svg>
  );
}
...
function AgentDetailContent() {
  ...
  const [activeTab, setActiveTab] = useState<"mcp" | "skills" | "prompt" | "model" | "tools">("mcp");
  ...
  const [selectedAllowTools, setSelectedAllowTools] = useState<string[]>([]);
  const [selectedDenyTools, setSelectedDenyTools] = useState<string[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setSelectedAllowTools(agentData.allow_tools || []);
      setSelectedDenyTools(agentData.deny_tools || []);
    ...
    try {
      ...
      if (agent.agent_type === "external") {
        ...
        await meowoneApi.upsertInternalAgent({
          ...
          mcp_servers: selectedMcp,
          agent_skills: selectedSkills,
          allow_tools: selectedAllowTools,
          deny_tools: selectedDenyTools,
          model_name: selectedModel,
          enabled: agent.enabled,
        });
      }
  ...
  const toggleAllowTool = (name: string) => {
    setSelectedAllowTools((prev) =>
      prev.includes(name) ? prev.filter((n) => n !== name) : [...prev, name]
    );
  };

  const toggleDenyTool = (name: string) => {
    setSelectedDenyTools((prev) =>
      prev.includes(name) ? prev.filter((n) => n !== name) : [...prev, name]
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-gray-200">
        ...
        >
          <ShieldIcon />
          工具限制
          {(selectedAllowTools.length > 0 || selectedDenyTools.length > 0) && (
            <span className="rounded-full bg-blue-100 px-2 py-0.5 text-xs text-blue-600">
              {(selectedAllowTools.length + selectedDenyTools.length)}
            </span>
          )}
        </button>
      </div>
        ...
        {activeTab === "tools" && (
          <div className="space-y-6">
            ...
            <div>
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-medium">允许的工具（白名单）</h3>
                  <p className="mt-1 text-sm text-gray-500">留空表示不限制。如果设置了白名单，则只能使用勾选的这些工具</p>
                </div>
                {selectedAllowTools.length > 0 && (
                  <span className="rounded-full bg-green-100 px-3 py-1 text-sm text-green-700">
                    已选择 {selectedAllowTools.length} 个
                  </span>
                )}
              </div>
              <div className="grid grid-cols-2 gap-2 md:grid-cols-3">
                {TOOL_LIST.map((tool) => {
                  const isSelected = selectedAllowTools.includes(tool.name);
                  return (
                    <button
                      key={tool.name}
            ...
            <div>
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-medium">禁止的工具（黑名单）</h3>
                  <p className="mt-1 text-sm text-gray-500">勾选的工具将被禁用，即使在白名单中也会被排除</p>
                </div>
                {selectedDenyTools.length > 0 && (
                  <span className="rounded-full bg-red-100 px-3 py-1 text-sm text-red-700">
                    已禁止 {selectedDenyTools.length} 个
                  </span>
                )}
              </div>
              <div className="grid grid-cols-2 gap-2 md:grid-cols-3">
                {TOOL_LIST.map((tool) => {
                  const isSelected = selectedDenyTools.includes(tool.name);
                  return (
                    <button
                      key={tool.name}
