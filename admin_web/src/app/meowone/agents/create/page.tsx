...
import type { ModelsListResponse, McpListResponse, SkillsListResponse, PromptsListResponse } from "@/lib/meowone-api";
...
const PRESET_TEMPLATES = [
  {
    ...
    name: "客服助手",
    icon: <MessageIcon />,
    description: "回答客户咨询，处理常见问题",
    systemPrompt: "你是一个专业、友好的客服助手。请用亲切的语气回答用户的问题，保持专业且耐心。如果不确定答案，请诚实告知用户。",
  },
  {
    id: "code_assistant",
    name: "代码助手",
    icon: <CodeIcon />,
    description: "编写、调试和解释代码",
    systemPrompt: "你是一个经验丰富的全栈开发工程师。请帮助用户编写高质量的代码，解释技术概念，并提供最佳实践建议。代码示例要清晰、可运行。",
  },
  {
    id: "data_analyst",
    name: "数据分析师",
    icon: <ChartIcon />,
    description: "分析数据，生成可视化建议",
    systemPrompt: "你是一个专业的数据分析师。擅长从数据中提取洞察，用简洁明了的方式解释统计结果，并提出数据驱动的建议。",
  },
  {
    id: "research_assistant",
    name: "研究助手",
    icon: <SearchIcon />,
    description: "搜索信息，整理研究资料",
    systemPrompt: "你是一个严谨的研究助手。擅长搜索和整理信息，帮助用户进行文献综述，提取关键观点，并以结构化的方式呈现研究成果。",
  },
  {
    id: "custom",
    name: "自定义",
    icon: <SparkleIcon />,
    description: "从零开始配置你的智能体",
    systemPrompt: "",
  },
];
...
export default function CreateAgentPage() {
  ...
  // 选择模板 - 模板现在只设置基本信息，不再自动设置提示词
  const handleSelectTemplate = (templateId: string) => {
    setSelectedTemplate(templateId);
    // 模板只记录选择，不再自动填充提示词（让用户从提示词库选择）
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // 如果选择了 promptKey，使用 promptKey；否则使用 customPrompt
      const systemPrompt = formData.promptKey 
        ? undefined // 将会通过 prompt_key 关联
        : formData.customPrompt;
      
      const promptKeyToSave = formData.promptKey || undefined;
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 1:
        return formData.modelName.length > 0 || defaultModelName;
      case 2:
      case 3:
      case 4:
        return true;
      ...
      <div className="sticky top-0 z-50 border-b border-gray-200 bg-white/80 backdrop-blur-md">
        <div className="mx-auto max-w-4xl px-4 py-4">
          <div className="mb-4 flex items-center justify-between">
            <h1 className="text-xl font-semibold text-gray-900">创建智能体</h1>
            ...
            {steps.map((step, index) => (
              <div key={step.id} className="flex items-center">
                ...
                {index < steps.length - 1 && (
                  <div
                    className={`mx-1 h-0.5 w-8 transition-all ${
                  ...
                  >
                    <div
                      className={`rounded-lg p-2 ${
                        selectedTemplate === template.id ? "bg-blue-500 text-white" : "bg-white text-gray-600"
                      }`}
                    >
                      {template.icon}
                    </div>
                ...
                <div className="space-y-2">
                  {models.map((model) => {
                    ...
                    return (
                      <button
                        key={modelName}
                        onClick={() => updateFormData("modelName", modelName)}
              ...
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    MCP (Model Context Protocol) 可以让你的智能体连接外部工具和数据
                  </p>
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <p className="mt-1 font-mono text-xs text-gray-400">{command}</p>
                        </div>
                      </button>
                    );
                  })}
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <div className="mb-4 flex gap-2 border-b border-gray-200 pb-2">
                <button
                  onClick={() => updateFormData("promptKey", "")}
              ...
              {formData.promptKey !== "__custom__" && (
                <div>
                  {loadingResources ? (
                    ...
                    <div className="py-8 text-center">
                      <p className="text-gray-500">暂无可用提示词模板</p>
                      <p className="mt-1 text-sm text-gray-400">
                        请先在「能力中心」-「提示词管理」中添加提示词
                      </p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                      {prompts.map((prompt) => {
                        ...
                        return (
                          <button
                            key={promptKey}
                            onClick={() => updateFormData("promptKey", promptKey)}
                            className={`flex items-start gap-3 rounded-xl border-2 p-4 text-left transition-all ${
                              isSelected
                                ? "border-blue-500 bg-blue-50"
                                : "border-gray-100 bg-gray-50 hover:border-gray-200"
          ...
          <div className="space-y-6">
            ...
            <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
              ...
              <div className="p-6">
                ...
                {/* 提示词预览 */}
                {formData.promptKey && prompts.find(p => p.prompt_key === formData.promptKey)?.content_md && (
                  <div className="mt-4">
                    <p className="mb-2 text-sm font-medium text-gray-700">提示词预览</p>
                    <div className="max-h-32 overflow-auto rounded-lg bg-gray-50 p-3 text-sm text-gray-600">
                      {prompts.find(p => p.prompt_key === formData.promptKey)?.content_md}
                    </div>
                  </div>
                )}
                {formData.customPrompt && (
                  <div className="mt-4">
                    <p className="mb-2 text-sm font-medium text-gray-700">自定义提示词预览</p>
                    <div className="max-h-32 overflow-auto rounded-lg bg-gray-50 p-3 text-sm text-gray-600">
                      {formData.customPrompt}
                    </div>
                  </div>
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-purple-600 px-6 py-4 font-medium text-white transition-all hover:from-blue-600 hover:to-purple-700 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {loading ? (
                <>
